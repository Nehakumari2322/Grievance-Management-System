...
class Reviewers extends Controller{
    ...
    public function __construct() {
       // echo 'executives construct';
        $this->agent = $this->model('Reviewer');
    }

    public function logmein()
    {
        $this->view('reviewers/login');
    } 
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if(isset($_POST['submit'])){
                ...
                if($matched == true ){
                   ...
                   $this->view('reviewers/dashboard',$data,$adata,$mdata,$ndata,$rdata,$sdata,$cdata,$ldata);
                }
                else{
                    // echo 'not matched';
                    $data = "password_mismatch";
                    $this->view('reviewers/login', $data);
                }
            }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            for ($count=0; $count<$totalcount; $count++) {
                if(isset($_POST['newstatus'.$count])){
                    ...
                    $adata['status'] = 'New';
                    $this->view('reviewers/subcategorycaselist',$data,$adata);
                }
    ...
    public function agentsaccess(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if(isset($_POST['cases'])){
                ...
                $this->view('reviewers/complain_personal_form',$data,$adata,$mdata,$ndata,$rdata);
            }
            else if(isset($_POST['request'])){
                $data = $this->agent->getAllWard();
                $adata = $this->agent->getAllBranch();
                $this->view('reviewers/requestform',$data,$adata);
            }
            else if(isset($_POST['newhigher'])){
                $data = $this->agent-> getNewHigherPriority();
                $adata['priority'] = 'Higher';
                $adata['status'] = 'New';
                $this->view('reviewers/priority',$data,$adata);
            }
                ...
                $data = $this->agent->new();
                $adata['status'] = 'New';
                $this->view('reviewers/casestatus',$data,$adata);
            }
                ...
                $this->view('reviewers/servicestatus',$data);
            }
    ...
    public function buildHTMLTextForPDFReport1($data){
        ...
        $html1=' <!DOCTYPE html>
         ...
         <tr>
        <th style="width:10%"><img src="img/logo1.jpg" height="80px"></th>
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $_POST = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
                if(isset($_POST['update'])){
                    ...
                    $this->view('reviewers/detailCasedetail',$data,$adata,$mdata);
            ...
            $_POST = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
                ...
                for($count=0; $count<=$totalcount; $count++) {
                    if(isset($_POST['update'.$count])){
                        ...
                        $data = $this->agent->getAllCasesDetails($Id);
                        $adata = $this->agent->getAllFieldAgent();
                        $this->view('reviewers/detailCasedetail',$data,$adata);
                    }
                }
            }
    }
